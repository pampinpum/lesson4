...
public class MainImpl implements Main {
    ...
    public String getLine1(AddressModel addressModel) {
        return Optional.ofNullable(addressModel).map(AddressModel::getLine1).filter(StringUtils::isNotBlank).orElse(StringUtils.EMPTY);
    }

    @Override
    public String getFullName(AddressModel addressModel) {
        StringBuilder stringBuilder = new StringBuilder();
        Optional.ofNullable(addressModel).map(AddressModel::getFirstName).filter(StringUtils::isNotBlank).ifPresent(stringBuilder::append);
        Optional.ofNullable(addressModel).map(AddressModel::getLastName).filter(StringUtils::isNotBlank)
                .ifPresent(stringBuilder.length() != 0 ? x -> stringBuilder.append(" " + x) : x -> stringBuilder.append(x));
    ...
    public AddressModel getBillingAddress(UserModel userModel) {
        ...
        return Optional.ofNullable(userModel).map(UserModel::getAddresses).filter(CollectionUtils::isNotEmpty)
                .map(x -> getAddress(x, a -> BooleanUtils.isTrue(a.getBillingAddress()))).orElse(null);
    ...
    public String getLastLoginFormatted(UserModel userModel) {
        ...
        return Optional.ofNullable(userModel).map(UserModel::getLastLogin).map((x -> format.format(x))).orElse("the user has not been logged yet");
    }
    ...
    public String getContactCountry(UserModel userModel) {
        ...
        return Optional.ofNullable(userModel).map(UserModel::getAddresses).filter(CollectionUtils::isNotEmpty)
                .map(x -> getAddress(x, a -> BooleanUtils.isTrue(a.getContactAddress()))).map(AddressModel::getCountry)
                .map(CountryModel::getIsocode).orElseGet(() -> inferCountry());
    ...
    public AddressModel getShippingAddress(UserModel userModel) throws AddressNotFoundException {
        ...
        return Optional.ofNullable(userModel).map(UserModel::getAddresses)
                .map(x -> getAddress(x, a -> BooleanUtils.isTrue(a.getShippingAddress()))).orElseThrow(AddressNotFoundException::new);
